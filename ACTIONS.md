...
## ACT-012 — CMS-driven blog, article migration, and premium design closeout

- **Priority:** P0 — Production release
- **Problem:** The blog was three hardcoded TSX route files. Publishing a new article required a code change, article copy was trapped in JSX, unknown article URLs returned a soft 404, and the editorial presentation did not match the rest of the site.
- **Root Cause:** No content management system existed. Article structure, copy, and presentation were all fused into per-article React components.
- **Solution:** Implemented Sanity as the content system (`apps/web/sanity/**`, `apps/web/sanity.config.ts`, Studio at `/studio`, preview and revalidation routes). Added an eighteen-block reusable editorial schema with a matching renderer for every block. Replaced the three route files with one `/blog/[slug]` route and a CMS-driven index. Migrated all three articles into structured content through a deterministic generator (`scripts/content/build-articles.mjs`) with an idempotent Sanity import (`scripts/sanity/import-articles.mjs`). Set `dynamicParams = false` on the article segment so unknown slugs return a genuine HTTP 404. Ran a premium design pass over the blog, footer, cards, and CTA system, and cleared every accessibility violation found.
- **Files Changed:** `apps/web/sanity/**`, `apps/web/sanity.config.ts`, `apps/web/app/blog/**`, `apps/web/app/studio/**`, `apps/web/app/api/preview/**`, `apps/web/app/api/revalidate/route.ts`, `apps/web/components/blog/**`, `apps/web/components/layout/site-footer.tsx`, `apps/web/lib/blog/**`, `apps/web/lib/site.ts`, `apps/web/app/sitemap.ts`, `apps/web/next.config.mjs`, `apps/web/content/articles/*.json`, `content/sanity/articles.ndjson`, `scripts/content/**`, `scripts/sanity/**`, `recovered-manus/build-blog.mjs`, `tests/static/*.test.mjs`, `qa-config/clientverse-audit.yaml`, `.github/workflows/clientverse-audit.yml`, `docs/05-content/SANITY_CMS.md`, `apps/web/.env.example`.
- **Validation:** Clean `pnpm install --frozen-lockfile`; typecheck, lint at zero warnings, 62 static tests, and a production build all pass. A Chromium crawl of 34 public routes reported zero bad statuses, zero broken internal links, zero console errors, zero images missing alternative text, and zero axe-core WCAG 2.1 AA violations. `/blog/<unknown>` returns HTTP 404 and all three preserved article URLs return HTTP 200. Responsive review captured at 375, 430, 768, 1024, and 1440 pixels.
- **Status:** Complete in the repository. Sanity project provisioning, the production domain cutover, and the ClientVerse audit run require external account access and remain BLOCKED.
